...
//        스프링에서 비동기 처리를 할 때 @RequestBody 와 @Responsebody 어노테이션을 사용
...
//        현쟂 로그인 유저의 정보를 얻기 위해서 @Controller 어노테이션이 선언된 클래스에서 메소드 인자로 principal
//        객체를 넘겨 줄 경우 해당 객체에 직접 접근할 수 있다.
...
//  주문번호를 받앗허 주문 취소 로직을 호출하는 메소드
    @PostMapping("/order/{orderId}/cancel")
    public @ResponseBody ResponseEntity cancelOrder(@PathVariable("orderId") Long orderId, Principal principal) {
        if(!orderService.validateOrder(orderId, principal.getName())) {
//            자바스크립트에서 취소할 주문 번호는 조작이 가능하므로 다른 사라므이 주문을 취소하지 못하도록 주문 취소 권한검사를 한다.
